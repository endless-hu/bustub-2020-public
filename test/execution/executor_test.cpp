...
// Additional test for Limit
TEST_F(ExecutorTest, SimpleSeqScanWithLimitTest) {
  // SELECT colA, colB FROM test_1 WHERE colA < 500 LIMIT 10
  ...
  auto *out_schema = MakeOutputSchema({{"colA", colA}, {"colB", colB}});
  SeqScanPlanNode plan{out_schema, predicate, table_info->oid_};
  LimitPlanNode limit_plan{&schema, &plan, (size_t)10, (size_t)0};
  // Execute
  std::vector<Tuple> result_set;
  GetExecutionEngine()->Execute(&limit_plan, &result_set, GetTxn(), GetExecutorContext());
...
// Addtional test for UpdateTuple
TEST_F(ExecutorTest, SimpleSeqScanWithUpdateTest) {
  // UPDATE test_1 SET colB = colB+10 WHERE colA < 20
  ...
  auto *colA = MakeColumnValueExpression(schema, 0, "colA");
  auto *colB = MakeColumnValueExpression(schema, 0, "colB");
  auto *const500 = MakeConstantValueExpression(ValueFactory::GetIntegerValue(20));
  auto *predicate = MakeComparisonExpression(colA, const500, ComparisonType::LessThan);
  auto *out_schema = MakeOutputSchema({{"colA", colA}, {"colB", colB}});
  SeqScanPlanNode scan_plan{out_schema, predicate, table_info->oid_};
  // construct update info, add colB by 10
  UpdateInfo add_colB_by10(UpdateType::Add, 10);
  const std::unordered_map<uint32_t, UpdateInfo> update_attr{{1, add_colB_by10}};
  UpdatePlanNode plan(&scan_plan, table_info->oid_, update_attr);
  // Execute
  std::vector<Tuple> result_set;
  GetExecutionEngine()->Execute(&plan, &result_set, GetTxn(), GetExecutorContext());
  ...
  // std::cout << "ColA, ColB" << std::endl;
  for (const auto &tuple : result_set) {
    ASSERT_TRUE(tuple.GetValue(out_schema, out_schema->GetColIdx("colA")).GetAs<int32_t>() < 20);
    ASSERT_TRUE(tuple.GetValue(out_schema, out_schema->GetColIdx("colB")).GetAs<int32_t>() >= 10);
    // std::cout << tuple.GetValue(out_schema, out_schema->GetColIdx("colA")).GetAs<int32_t>() << ", "
              // << tuple.GetValue(out_schema, out_schema->GetColIdx("colB")).GetAs<int32_t>() << std::endl;
  }
  ASSERT_EQ(result_set.size(), 20);
}
...
TEST_F(ExecutorTest, SimpleDeleteTest) {
  ...
  /*static_cast<BPlusTreeIndex<GenericKey<8>, RID, GenericComparator<8>> *>(index_info->index_.get())
      ->container_.Draw(GetExecutorContext()->GetBufferPoolManager(), "before_del");*/
  // Verify
  ...
  Tuple index_key = Tuple(result_set[0]);  // it seems that this line is incorrect
  // Because it obviously INTENDS to create a key from the tuple in the result_set
  // However, it ACTUALLY replicates the tuple, which will cause error in :
  // index_info->index_->ScanKey(index_key, &rids, GetTxn());
  // add the following single line to correct the mistake
  index_key = index_key.KeyFromTuple(schema, *key_schema, index_info->index_->GetKeyAttrs());
  // LOG_INFO("index key: %s", index_key.ToString(key_schema).data());
  ...
  /*static_cast<BPlusTreeIndex<GenericKey<8>, RID, GenericComparator<8>> *>(index_info->index_.get())
      ->container_.Draw(GetExecutorContext()->GetBufferPoolManager(), "after_del");*/
  result_set.clear();
  GetExecutionEngine()->Execute(scan_plan1.get(), &result_set, GetTxn(), GetExecutorContext());
...
// Addtional test for nested index join
TEST_F(ExecutorTest, SimpleNestedIndexJoinTest) {
  ...
  const Schema *out_schema2;
  {
    ...
    // create index for test_2 on col1
    Schema *key_schema = ParseCreateStatement("a bigint");
    GenericComparator<8> comparator(key_schema);
    GetExecutorContext()->GetCatalog()->CreateIndex<GenericKey<8>, RID, GenericComparator<8>>(
        GetTxn(), "index1", "test_2", table_info->schema_, *key_schema, {0}, 8);
  ...
  std::unique_ptr<NestedIndexJoinPlanNode> index_join_plan;
  const Schema *out_final;
  {
    // colA and colB have a tuple index of 0 because they are the left side of the join
    auto colA = MakeColumnValueExpression(*out_schema1, 0, "colA");
    ...
    auto predicate = MakeComparisonExpression(colA, col1, ComparisonType::Equal);
    out_final = MakeOutputSchema({{"colA", colA}, {"colB", colB}, {"col1", col1}, {"col3", col3}});
    index_join_plan = std::make_unique<NestedIndexJoinPlanNode>(
        out_final, std::vector<const AbstractPlanNode *>{scan_plan1.get()}, predicate,
        GetExecutorContext()->GetCatalog()->GetTable("test_2")->oid_, "index1", out_schema1, out_schema2);
  }

  std::vector<Tuple> result_set;
  GetExecutionEngine()->Execute(index_join_plan.get(), &result_set, GetTxn(), GetExecutorContext());
  ASSERT_EQ(result_set.size(), 100);
  // std::cout << "ColA, ColB, Col1, Col3" << std::endl;
  // for (const auto &tuple : result_set) {
    // std::cout << tuple.GetValue(out_final, out_final->GetColIdx("colA")).GetAs<int32_t>() << ", "
...
TEST_F(ExecutorTest, SimpleAggregationTest) {
  ...
  const Schema *agg_schema;
  {
    ...
    std::vector<const AbstractExpression *> agg_exprs{countA, sumA, minA, maxA};
...
TEST_F(ExecutorTest, SimpleGroupByAggregation) {
  ...
  const Schema *agg_schema;
  {
    ...
    std::vector<const AbstractExpression *> aggr_exprs{countA, groupbyB, sumC};
    agg_plan =
        std::make_unique<AggregationPlanNode>(agg_schema, scan_plan.get(), having, std::move(group_by_cols),
                                              std::move(aggregate_cols), std::move(agg_types), std::move(aggr_exprs));
  }
